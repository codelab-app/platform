...
;// ../../libs/backend/infra/adapter/cli/src/commands/kubernetes/kubernetes.service.ts
...
let KubernetesService = class KubernetesService {
    ...
    builder(yargv) {
        return yargv
            ...
            .command('init', 'Create and bootstrap Kubernetes cluster', (argv) => argv, globalHandler(({ stage }) => {
            const environment = this.getEnvironmentName(stage);
            console.log(`🚀 Initializing Kubernetes cluster for ${environment}...`);
            // Create cluster
            ...
            .command('deploy', 'Deploy applications to Kubernetes', (argv) => argv.option('tag', {
            ...
            .command('migrate', 'Migrate data to Kubernetes (Neo4j)', (argv) => argv, globalHandler(({ stage }) => {
            const environment = this.getEnvironmentName(stage);
            const sourceHost = this.getSourceHost(stage);
            console.log(`🔄 Migrating Neo4j data from ${sourceHost} to Kubernetes...`);
            $stream.sync `./infra/kubernetes/scripts/migration/migrate-neo4j.sh ${environment} ${sourceHost}`;
            console.log('✅ Data migration completed successfully');
        }))
            .command('port-forward', 'Port forward a service to localhost', (argv) => argv
            .option('service', {
            ...
            console.log(`🔌 Port forwarding ${service} from ${environment} to localhost:${localPort}`);
            if (service === 'neo4j') {
            ...
            .command('logs', 'View logs for a service', (argv) => argv.option('service', {
            ...
            const environment = this.getEnvironmentName(stage);
            const selector = service === 'neo4j' ? 'statefulset/neo4j' : `deployment/${service}`;
            console.log(`📜 Streaming logs for ${service} in ${environment}...`);
            $stream.sync `kubectl -n ${environment} logs -f ${selector}`;
        }))
            .command('scale', 'Scale a deployment', (argv) => argv
            .option('service', {
            ...
            const environment = this.getEnvironmentName(stage);
            console.log(`⚖️ Scaling ${service} to ${replicas} replicas in ${environment}...`);
            ...
            .command('rollback', 'Rollback deployments to previous version', (argv) => argv, globalHandler(({ stage }) => {
            const environment = this.getEnvironmentName(stage);
            ...
            .command('cleanup', 'Clean up Kubernetes resources', (argv) => argv, globalHandler(({ stage }) => {
            const environment = this.getEnvironmentName(stage);
            console.log(`🧹 Cleaning up Kubernetes resources in ${environment}...`);
            ...
            .command('status', 'Show cluster and deployment status', (argv) => argv, globalHandler(({ stage }) => {
            const environment = this.getEnvironmentName(stage);
            ...
            .command('secrets', 'Create or update secrets', (argv) => argv, globalHandler(({ stage }) => {
            const environment = this.getEnvironmentName(stage);
    ...
    updateImageTags(environment, tag) {
    ...
    waitForDeployments(environment) {
...
KubernetesService = (0,external_tslib_namespaceObject.__decorate)([
    (0,common_namespaceObject.Injectable)(),
    (0,external_tslib_namespaceObject.__metadata)("design:paramtypes", [])
], KubernetesService);
...
let CommandService = class CommandService {
    ...
    terraformService, taskService) {
        ...
        Object.defineProperty(this, "kubernetesService", {
            enumerable: true,
            configurable: true,
            writable: true,
