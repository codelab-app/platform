...
export interface FragmentMaskingConfig {
...
export interface ClientPresetConfig {
  /**
...
export const preset: Types.OutputPreset<ClientPresetConfig> = {
  buildGeneratesSection: (options) => {
    ...
    const isPersistedOperations =
      Boolean(options.presetConfig.persistedDocuments) ?? false
    ...
    const reexportsExtension = options.config.emitLegacyCommonJSImports
      ? ''
      : '.js'
        ...
        plugins: [
          {
            ['add']: {
              content: reexports
                ...
                .map(
                  (moduleName) =>
                    `export * from "./${moduleName}${reexportsExtension}";`,
                )
                .join('\n'),
            },
          },
