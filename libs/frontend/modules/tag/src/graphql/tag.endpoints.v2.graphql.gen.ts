...
export type GetTagGraphsQuery = { tagGraphs?: TagGraphFragment | null | undefined };
...
export const GetTagGraphsGql = gql`
  ...
  const injectedRtkApi = api.injectEndpoints({
    endpoints: (build) => ({
      CreateTags: build.mutation<CreateTagsMutation, GraphqlOperationOptions<CreateTagsMutationVariables>>({
        query: (options) => ({ document: CreateTagsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      UpdateTags: build.mutation<UpdateTagsMutation, GraphqlOperationOptions<UpdateTagsMutationVariables>>({
        query: (options) => ({ document: UpdateTagsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      DeleteTags: build.mutation<DeleteTagsMutation, GraphqlOperationOptions<DeleteTagsMutationVariables>>({
        query: (options) => ({ document: DeleteTagsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      GetTags: build.query<GetTagsQuery, GraphqlOperationOptions<GetTagsQueryVariables> | void | undefined>({
        query: (options) => ({ document: GetTagsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      GetTagGraphs: build.query<GetTagGraphsQuery, GraphqlOperationOptions<GetTagGraphsQueryVariables> | void | undefined>({
        query: (options) => ({ document: GetTagGraphsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
    }),
  });
  export { injectedRtkApi as api };
  export const { useCreateTagsMutation, useUpdateTagsMutation, useDeleteTagsMutation, useGetTagsQuery, useLazyGetTagsQuery, useGetTagGraphsQuery, useLazyGetTagGraphsQuery } = injectedRtkApi;
