...
export const DeleteHooksGql = gql`
  ...
  const injectedRtkApi = api.injectEndpoints({
    endpoints: (build) => ({
      CreateHooks: build.mutation<CreateHooksMutation, GraphqlOperationOptions<CreateHooksMutationVariables>>({
        query: (options) => ({ document: CreateHooksGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      DeleteHooks: build.mutation<DeleteHooksMutation, GraphqlOperationOptions<DeleteHooksMutationVariables>>({
        query: (options) => ({ document: DeleteHooksGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
    }),
  });
