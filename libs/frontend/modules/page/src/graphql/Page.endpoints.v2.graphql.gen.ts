...
export const GetPagesGql = gql`
  ...
  const injectedRtkApi = api.injectEndpoints({
    endpoints: (build) => ({
      CreatePages: build.mutation<CreatePagesMutation, GraphqlOperationOptions<CreatePagesMutationVariables>>({
        query: (options) => ({ document: CreatePagesGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      DeletePages: build.mutation<DeletePagesMutation, GraphqlOperationOptions<DeletePagesMutationVariables> | void | undefined>({
        query: (options) => ({ document: DeletePagesGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      UpdatePages: build.mutation<UpdatePagesMutation, GraphqlOperationOptions<UpdatePagesMutationVariables> | void | undefined>({
        query: (options) => ({ document: UpdatePagesGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      GetPages: build.query<GetPagesQuery, GraphqlOperationOptions<GetPagesQueryVariables> | void | undefined>({
        query: (options) => ({ document: GetPagesGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
    }),
  });
  export { injectedRtkApi as api };
  export const { useCreatePagesMutation, useDeletePagesMutation, useUpdatePagesMutation, useGetPagesQuery, useLazyGetPagesQuery } = injectedRtkApi;
