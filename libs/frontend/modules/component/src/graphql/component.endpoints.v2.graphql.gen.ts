...
export const GetComponentsGql = gql`
  ...
  const injectedRtkApi = api.injectEndpoints({
    endpoints: (build) => ({
      CreateComponents: build.mutation<CreateComponentsMutation, GraphqlOperationOptions<CreateComponentsMutationVariables>>({
        query: (options) => ({ document: CreateComponentsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      DeleteComponents: build.mutation<DeleteComponentsMutation, GraphqlOperationOptions<DeleteComponentsMutationVariables> | void | undefined>({
        query: (options) => ({ document: DeleteComponentsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      UpdateComponents: build.mutation<UpdateComponentsMutation, GraphqlOperationOptions<UpdateComponentsMutationVariables> | void | undefined>({
        query: (options) => ({ document: UpdateComponentsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
      GetComponents: build.query<GetComponentsQuery, GraphqlOperationOptions<GetComponentsQueryVariables> | void | undefined>({
        query: (options) => ({ document: GetComponentsGql, options: {...{"context":{"env":"v2"}}, ...options}, })
      }),
    }),
  });
  export { injectedRtkApi as api };
  export const { useCreateComponentsMutation, useDeleteComponentsMutation, useUpdateComponentsMutation, useGetComponentsQuery, useLazyGetComponentsQuery } = injectedRtkApi;
