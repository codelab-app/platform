...
// returns true if PATH_TEMPLATE_START_OPEN_REGEX finds something in the text on the left of the cursor
const checkLeftBracket = (context: CompletionContext): boolean => {
  ...
  const found = str.search(EXP_PATH_TEMPLATE_START_OPEN_REGEX)
...
export const completionSource: CompletionExtension = (input) => {
  ...
  return (context) => {
    ...
    return {
      ...
      options,
      validFor: new RegExp(
        `^([\\w$]*)|(${EXP_PATH_TEMPLATE_START_REGEX.toString()})$`,
      ),
    }
  }
}
