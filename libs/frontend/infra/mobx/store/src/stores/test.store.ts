...
// interface ITestRootStore extends IRootStore {
//   setupPage(
...
export class TestRootStore
  extends Model({
    applicationStore: prop<IApplicationStore>(),
    domainStore: prop<IDomainStore>(),
  })
  implements IRootStore
{
  ...
  addRenderer(dto: Partial<IRendererDto>) {
    ...
    const renderer = rendererFactory(this.applicationStore.rendererService)(dto)
  ...
  setupPage(
    ...
    const pages = pageKind === IPageKind.Regular ? [providerPage, page] : [page]
  ...
  setupRuntimeComponent() {
    ...
    const component = componentFactory(this.domainStore.componentDomainService)(
      {
        id: componentId,
        name: 'Component',
  ...
  protected onAttachedToRootStore() {
    const richTextType = this.addRichTextType({})
    const api = this.addInterfaceType({})
  ...
  protected onInit(): void {
    ...
    elementDomainServiceContext.set(this, this.domainStore.elementDomainService)
