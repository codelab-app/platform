 ...
 */
export const convertToPackageName = (projectName: string): string => {
  ...
  if (
    projectName.startsWith('@codelab/frontend-') &&
    projectName.split('/').length > 2
  ) {
    // Keep original name with slashes
    console.log(
      `convertToPackageName output (frontend multi-segment): ${packageName}`,
    )
  ...
  if (projectName.match(/^@codelab\/backend-infra-adapter\/[^/]+$/)) {
    ...
    console.log(
      `convertToPackageName output (backend-infra-adapter): ${packageName}`,
    )
