...
build {
  ...
  provisioner "file" {
    content     = templatefile("templates/doctl-config.yaml.tpl", {
  ...
  provisioner "file" {
    source      = "files/consul-template-daemon.hcl"
    destination = "/etc/consul-template/consul-template-daemon.hcl"
  }

  provisioner "file" {
    source      = "files/daemon.json.ctmpl"
  ...
  provisioner "file" {
    source      = "files/docker-loki-plugin.service"
    destination = "/etc/systemd/system/docker-loki-plugin.service"
  }

  provisioner "file" {
    source      = "files/docker-daemon-config.service"
    destination = "/etc/systemd/system/docker-daemon-config.service"
  }
  ...
  provisioner "shell" {
    inline = [
      ...
      "systemctl enable docker consul docker-loki-plugin docker-daemon-config docker-login consul-template",
      "",
