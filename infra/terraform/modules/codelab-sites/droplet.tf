# Find the latest Packer-built sites image
data "digitalocean_images" "codelab_sites" {
  filter {
    key      = "name"
    values   = ["codelab-sites"]
    match_by = "substring"
  }
  filter {
    key    = "regions"
...
resource "digitalocean_droplet" "codelab_sites" {
  image  = data.digitalocean_images.codelab_sites.images[0].id
  name   = "sites"
