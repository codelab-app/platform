...
describe('In-app navigation between app pages', () => {
  ...
  describe('Preview Mode', () => {
    ...
    it('should navigate to the static page when NextLink in the _app is clicked', () => {
      cy.get('#render-root')
        .contains(providerPageLinkElementCreateData.propsData?.children.value)
        .click()
      cy.contains(
        staticPageTextElementCreateData.propsData?.children.value,
      ).should('exist')
    })

    it('should navigate to the dynamic page when NextLink in the static page is clicked', () => {
      cy.get('#render-root')
        .contains(staticPageLinkElementCreateData.propsData?.children.value)
        .click()
      cy.findByText(
  ...
  describe('Builder Mode', () => {
    ...
    it('should disable navigation', () => {
      ...
      cy.wait(2000)
      cy.contains(
        providerPageLinkElementCreateData.propsData?.children.value,
      ).should('exist')
      cy.contains(
        staticPageTextElementCreateData.propsData?.children.value,
      ).should('not.exist')
