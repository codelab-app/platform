...
export const POST = async (request: NextRequest): Promise<NextResponse> => {
  try {
    ...
    const discussions = await githubService.fetchDiscussions(owner, repo)
    // Check sync status if mem0ApiKey is provided
    let discussionsWithSync = discussions

    if (mem0ApiKey) {
      const mem0Client = createMem0Client(mem0ApiKey)
      const mem0Service = new Mem0Service(mem0Client)

      discussionsWithSync = await mem0Service.checkSyncStatus(
        discussions,
        owner,
        repo,
    ...
    return NextResponse.json({
      discussions: discussionsWithSync,
      total: discussionsWithSync.length,
    })
  } catch (error) {
