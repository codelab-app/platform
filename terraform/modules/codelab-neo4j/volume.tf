...
resource "digitalocean_volume" "neo4j_prometheus" {
  region                  = var.digitalocean_region
  name                    = "codelab-neo4j-prometheus-volumes"
  size                    = 10
  initial_filesystem_type = "ext4"
  # description             = "an example volume"
}
...
resource "digitalocean_volume" "neo4j_alloy" {
  region                  = var.digitalocean_region
  name                    = "codelab-neo4j-alloy-volumes"
  size                    = 10
  initial_filesystem_type = "ext4"
  # description             = "an example volume"
}

resource "digitalocean_volume_attachment" "neo4j_alloy" {
  droplet_id = digitalocean_droplet.neo4j.id
  volume_id  = digitalocean_volume.neo4j_alloy.id
